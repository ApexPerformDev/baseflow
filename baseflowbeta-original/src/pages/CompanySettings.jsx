...
export default function CompanySettings() {
  ...
  const { data: storeDetails, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (store) {
        setFormData({
          name: store.name || '',
          email_empresa: store.email_empresa || '',
  ...
  const updateStoreMutation = useMutation({
    ...
    onSuccess: (updatedStore) => {
      ...
      setCurrentStore(prev => ({ ...prev, name: updatedStore.name }));
  ...
  const handleLogoUpload = async (e) => {
    ...
    try {
      ...
      const updatedStore = await base44.entities.Store.update(currentStore.id, { logo_url: file_url });
      
      // Atualizar localStorage
      const updatedStoreData = { ...currentStore, logo_url: file_url };
      localStorage.setItem('currentStore', JSON.stringify(updatedStoreData));
      setCurrentStore(updatedStoreData);
