...
export default function Settings() {
  ...
  const { data: storeUsers = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!currentStore?.id) return [];
      return await base44.entities.StoreUser.filter({
        store_id: currentStore.id,
      });
    },
    enabled: !!currentStore?.id,
  ...
  const integrationsList = [
    {
      ...
      name: "Nuvemshop",
      description:
        "Conecte sua loja Nuvemshop para importar pedidos e clientes automaticamente",
      icon: Store,
    },
    {
      type: "BLING",
      name: "Bling ERP",
      description:
        "Integre com o Bling para sincronizar produtos, pedidos e estoque",
      icon: SettingsIcon,
    },
  ];
...
🕒 Última sincronização: ${
        ...
        if (
          errorMsg.includes("Token de acesso inválido") ||
...
Última Sincronização: ${
        data.integration.lastSyncAt
          ? new Date(data.integration.lastSyncAt).toLocaleString("pt-BR")
          : "Nunca"
      }
...
- Último Pedido: ${
        data.statistics.lastOrderDate
          ? new Date(data.statistics.lastOrderDate).toLocaleString("pt-BR")
          : "N/A"
      }
      `.trim();
  ...
  const handleTestAuth = async () => {
    try {
      ...
      const data = response.data || response;
      console.log("[DEBUG_AUTH] Resposta:", data);
      ...
      if (data.success) {
        ...
        details += `- Token salvo em: ${new Date(
          data.token_saved_at
        ...
        details += `- Nuvemshop Store ID: ${
          data.nuvemshop_store_id || "N/A"
        }\n`;
  ...
  const handleDebugCounts = async () => {
    try {
      ...
      const data = response.data || response;
      console.log("[DEBUG_COUNTS] Resposta:", data);
      ...
      details += `- Última sync: ${
        data.integration.last_sync
          ? new Date(data.integration.last_sync).toLocaleString("pt-BR")
          : "Nunca"
      }\n`;
      ...
      if (data.latest_data.orders.length > 0) {
        ...
        data.latest_data.orders.forEach((o) => {
          details += `- #${o.id}: R$ ${o.amount} (${new Date(
            o.date
  ...
  const handleDebugLastAuth = async () => {
    try {
      ...
      const data = response.data || response;
      console.log("[DEBUG_LAST_AUTH] Resposta:", data);
      ...
      details += `🔑 Token Info:\n`;
      details += `- Token recebido: ${
        data.token_info.access_token_received ? "Sim" : "Não"
      }\n`;
      details += `- Tamanho: ${data.token_info.token_length} chars\n`;
      details += `- Preview: ${data.token_info.token_preview}\n`;
      details += `- Salvo em: ${new Date(
        data.token_info.token_saved_at
      ).toLocaleString("pt-BR")}\n\n`;
      ...
      if (data.token_validation) {
        ...
        details += `- Válido: ${
          data.token_validation.is_valid ? "Sim ✅" : "Não ❌"
        }\n`;
        details += `- Resultado: ${data.token_validation.status_text}\n`;
        if (data.token_validation.error_body) {
          details += `- Erro: ${data.token_validation.error_body.substring(
            0,
            100
          )}...\n`;
        }
            ...
            <Card className="bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200">
              <CardContent className="p-6">
                <p className="text-sm text-yellow-800 dark:text-yellow-200">
                  <strong>Permissão Negada:</strong> Apenas administradores
                  podem gerenciar integrações. Entre em contato com um admin da
                  loja para fazer alterações.
                </p>
              </CardContent>
            </Card>
          )}
